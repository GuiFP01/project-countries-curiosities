...
// Sort the countries in alphabetic order, starting with "a"

countries.sort((a, b) => a.name.official.localeCompare(b.name.official));

// Calling the select bar from html to JS and put a event listener

document.querySelector("#worldCountries").innerHTML = [`<option value="">Select country</option>`, ...countries.map((country) => `<option value="${country.name.common}">${country.name.official}</option>`)];


document.querySelector("#worldCountries").addEventListener("change", (event) => {
    selectedCountry(event.target.value, ".country-informations-js");
})

// General function to connect countries informations with the html elements

function selectedCountry (countryValue, parentClassName) {

    const theCountry = countries.find(country => country.name.common === countryValue);

    document.querySelector(`${parentClassName} .flag-image-js`).setAttribute("src", theCountry.flags.png);
    document.querySelector(`${parentClassName} .country-name-js`).innerHTML = theCountry.name.official;
    document.querySelector(`${parentClassName} .capital-name-js`).innerHTML = theCountry.capital;
    document.querySelector(`${parentClassName} .population-js`).innerHTML = theCountry.population;
}
...
// Extra countries array

// const extraCountriesList = [...countries];

// // Biggest country

// const areaValue = extraCountriesList.sort((a,b) => b.area - a.area)[0];
// selectedCountry(areaValue.name.common, ".biggest-country-js");



// // Country with the biggest population

// const populationValue = extraCountriesList.sort((a,b) => b.population - a.population)[0];
// selectedCountry(populationValue.name.common, ".biggest-population-js");


// // Number of countries in each continent

// const countriesEachContinent = countries.reduce( (acc, curr) => Object.keys(acc).includes(curr.region) ? acc[curr.region] = acc[curr.region]+1 : acc[curr.region] = 1 ,{});

// console.log(countriesEachContinent);
